...
| Y11 | Rule table | The safety authority's CSV — the handoff's twelve columns verbatim, the nine compile columns filled at the sitting — through the **Rule table** card on Data Sources (Export CSV hands them the seed in that layout to start from); the dry run reports what each row fires on today and which spaces change state; Confirm needs `commit_document`, **Sign this table** needs `sign_rule_table` (Safety alone) and ledgers `RULE_TABLE_SIGNED` with the hash and every version; until then the seed | `reference/cvn73/CVN73-rule-table.csv` (the seed exported, `SEED` on the card); `handoff/01-rule-table.csv`; brief in `docs/briefs/safety-authority-sitting.md` | The card reads `SIGNED` under the authority's name with `rows_in_force` as agreed and no *unbound* work type they did not accept; every row in force has its golden trace in `crates/wadl-engine/tests/snapshots/`, or the signed hot-work-only fallback is on file |
...
| 10 | Rule table (Y11) | sitting held | the dry run's *fires on* and *spaces that change* read row by row with the authority; the card `SIGNED` under their name, or the fallback signed |
