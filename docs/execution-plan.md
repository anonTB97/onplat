...
| 10 | **A person behind every decision** (S12, server sitting) | landed `7a12751` (store), `S12-ROLES-HASH` (API) | The identity hop carries a person: `x-wadl-person` (the proxy's stable subject, required in proxy mode — 401 `"the proxy asserted no person (x-wadl-person)"` without it, 401 outside `[A-Za-z0-9._:@/-]{1,128}` in either mode), `x-wadl-person-name` (percent-encoded, display only, falls back to the id with a `whoami` warning) and `x-wadl-roles` (eight codes; unknown codes ignored and reported; absent → `WADL_DEFAULT_ROLES` or `reader` behind the proxy, every door on the dev shim with the warning `demo mode: no x-wadl-roles — every door is open`). `TenantScope` carries an `Actor { id, name, source }`; every ledger row on both stores is chain format 2 with the actor in the hash (migration 0017), the rows before are format 1 and verify in the same chain, `wadl verify-ledger` reads old and new exports. One role → capability matrix (`roles.rs`: `read · raise_hazard · clear_hazard · commit_document · propose · decide`) and one `route_layer` gate over every POST (23 routes, the yard-clock door included): a role without the capability is refused 403 problem+json `{"detail":"Foreman may not record a clearance — clear_hazard is held by Ship Super and Safety","capability":"clear_hazard","roles":["foreman"]}`, nothing written; a dry run is never gated; a foreign hull is 404 before any capability is judged. `/api/whoami` serves person, roles, capabilities, hulls, the matrix, warnings and `WADL_MARKINGS`; `/health` names the identity mode; the audit line carries `person`. Generated weakest-role tests (one per gated route, as a reader), `identity.rs`, `pg_rls` round trip, SSP AC-6/AU-10/IA-2 statements, WADL-SA-11. Deferred: the shell half (boot from `/health` + `/api/whoami`, `DEMO MODE` badge, By column, capability-aware buttons, `docs/identity-proxy-contract.md`, POA&M and posture text) is sitting two; the shell still sends the two dev headers and lands as `dev:anonymous`. |
