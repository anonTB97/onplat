...
| 3 | **Reports** | landed | A Reports module: shift sheet, zone day sheet, compartment card, conflict log, field-condition register — pure builders (`reports.ts`, unit-tested), one screen, monochrome print, CSV with the cut in its header. Every sheet carries hull, instant, schedule source and producer, and names the layer of each figure. |
| 4 | **The ship through the product** | next | Booked hours derived from the ingested schedule instead of seeded work orders; a hazard-raise door and a daily hazard-log CSV door; compartment-register and coupling doors in the Data Sources home, with derived vertical adjacency; every door commit and revert ledgered. |
