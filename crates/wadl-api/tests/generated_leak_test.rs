...
#[tokio::test]
async fn leak_post_api_vessels_id_hazards() {
    let (_, w) = wadl_api::demo_app();
    let org = w.yard_org.as_uuid().to_string();
    let assigned = yard_assigned(&w);
    let foreign = w.navy_hull.as_uuid().to_string();
    ...
    assert_eq!(
        code,
        StatusCode::NOT_FOUND,
        "cross-tenant POST /api/vessels/:id/hazards must be 404"
    );
}
...
#[tokio::test]
async fn leak_get_api_vessels_id_zones_zone_adjacent() {
    let (_, w) = wadl_api::demo_app();
    let org = w.yard_org.as_uuid().to_string();
    let assigned = yard_assigned(&w);
    let foreign = w.navy_hull.as_uuid().to_string();
    let path = "/api/vessels/:id/zones/:zone/adjacent"
        .replace(":id", &foreign)
        .replace(":no", "4-141-0-C");
    let code = status("GET", &path, &org, &assigned, None).await;
    assert_eq!(
        code,
        StatusCode::NOT_FOUND,
        "cross-tenant GET /api/vessels/:id/zones/:zone/adjacent must be 404"
    );
}
...
async fn leak_get_api_vessels_id_schedule_proposals() {
    ...
    assert_eq!(
        code,
        StatusCode::NOT_FOUND,
        "cross-tenant GET /api/vessels/:id/schedule-proposals must be 404"
    );
}

#[tokio::test]
async fn leak_post_api_vessels_id_schedule_proposals() {
    let (_, w) = wadl_api::demo_app();
    let org = w.yard_org.as_uuid().to_string();
    let assigned = yard_assigned(&w);
    let foreign = w.navy_hull.as_uuid().to_string();
    ...
    assert_eq!(
        code,
        StatusCode::NOT_FOUND,
        "cross-tenant POST /api/vessels/:id/schedule-proposals must be 404"
    );
}

#[tokio::test]
async fn leak_post_api_vessels_id_schedule_proposals_withdraw() {
    let (_, w) = wadl_api::demo_app();
    let org = w.yard_org.as_uuid().to_string();
    let assigned = yard_assigned(&w);
    let foreign = w.navy_hull.as_uuid().to_string();
    ...
    assert_eq!(
        code,
        StatusCode::NOT_FOUND,
        "cross-tenant POST /api/vessels/:id/schedule-proposals/withdraw must be 404"
    );
}
