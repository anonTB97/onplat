...
| 4 | **The ship through the product** | in progress — `2000cf7` landed the first half | Landed: booked hours derived from the ingested schedule instead of seeded work orders (the readiness rollup says which, `hours_source`); a hazard-raise route with validation against the register and a `HAZARD_RAISED` ledger entry, raised from the deck plan beside the clearance. Next: a daily hazard-log CSV door; compartment-register and coupling doors in the Data Sources home, with derived vertical adjacency; every door commit and revert ledgered. |
...
| 6 | **Demo hardening** | planned | API integration suite run against PostgreSQL in CI; one demo seed for both stores; `schema_version` on stored documents; `/health` that checks the database; a scripted demo path over the sample schedule; a second engineer's cold walkthrough. |
